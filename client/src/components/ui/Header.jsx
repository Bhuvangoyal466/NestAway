...
export const Header = () => {
  ...
  return (
    <header
      className={`fixed top-0 z-10 flex w-screen justify-center py-4 backdrop-blur-md transition-all ${
        hasShadow ? 'shadow-lg' : ''
      }`}
    >
      <div
        className={`flex w-screen max-w-screen-xl ${
          showSearchBar ? 'justify-around' : 'justify-between px-10'
        }`}
      >
        <a href="/" className="flex items-center gap-1">
          <img className="h-8 w-8 md:h-10 md:w-10" src="./logo.png" alt="" />
          <span className="hidden text-2xl font-bold text-white md:block">
            NestAway
          </span>
        </a>
        ...
        <Link
          ...
          className="w-50 border-10 flex h-full items-center gap-2 rounded-full border-white px-2 py-1 md:border"
        >
          <svg
            ...
            {user ? (
              <Avatar>
                <AvatarImage
                  src={
                    user?.picture ||
                    'https://res.cloudinary.com/divjsroxw/image/upload/v1698205532/journeyscape_DEV/f7ij6fte64btdmkzdhbx.jpg'
                  }
